...
<p align="center">
  <img src="./shell-config.svg" alt="shell-config logo" width="128" />
</p>

<h1 align="center">@sebastienrousseau/shell-config</h1>
...
<p align="center">
  <a href="https://github.com/sebastienrousseau/shell-config/actions"><img src="https://img.shields.io/github/actions/workflow/status/sebastienrousseau/shell-config/ci.yml?branch=main&style=for-the-badge&logo=github" alt="Build Status" /></a>
  <a href="https://www.npmjs.com/package/@sebastienrousseau/shell-config"><img src="https://img.shields.io/npm/v/%40sebastienrousseau%2Fshell-config.svg?style=for-the-badge&color=fc8d62&logo=npm" alt="npm package" /></a>
  <a href="https://sebastienrousseau.com/shell-config/"><img src="https://img.shields.io/badge/docs-sebastienrousseau.com-brightgreen.svg?style=for-the-badge&logo=github" alt="Documentation" /></a>
  <a href="https://scorecard.dev/viewer/?uri=github.com/sebastienrousseau/shell-config"><img src="https://img.shields.io/ossf-scorecard/github.com/sebastienrousseau/shell-config?style=for-the-badge&label=OpenSSF%20Scorecard&logo=openssf" alt="OpenSSF Scorecard" /></a>
...
- [Strict ShellCheck Analysis & shfmt Formatting](#strict-shellcheck-analysis-shfmt-formatting) — before and after rule comparison
...
Install `@sebastienrousseau/shell-config` as a development dependency:
...
# pnpm
pnpm add -D @sebastienrousseau/shell-config

# yarn
yarn add -D @sebastienrousseau/shell-config

# bun
bun add -d @sebastienrousseau/shell-config
```

### Build from source

```bash
git clone https://github.com/sebastienrousseau/shell-config.git
cd shell-config
make            # check + test
```
...
- **Target environment**: Optimized for POSIX / Bash / ShellCheck.
...
```bash
cp node_modules/@sebastienrousseau/shell-config/.shellcheckrc ./.shellcheckrc
cp node_modules/@sebastienrousseau/shell-config/.shfmt ./.shfmt
```

### Or point the tool at the installed file

```bash
shellcheck --rcfile=node_modules/@sebastienrousseau/shell-config/.shellcheckrc script.sh
```
...
`@sebastienrousseau/shell-config` is a focused satellite of the [`@sebastienrousseau/config`](https://github.com/sebastienrousseau/config) suite — a unified ecosystem of single-purpose, zero-overhead developer configurations designed to share a single design philosophy, strict typing, and zero runtime dependencies.
...
`@sebastienrousseau/shell-config` solves this with three deliberate engineering choices:
...
## Strict ShellCheck Analysis & shfmt Formatting

Guarantees bash and POSIX sh script safety by catching unquoted variables, broken pipe handling, and non-portable shell idioms.

### Before (Unstandardized)

```text
#!/bin/bash
# Unchecked shell script
```

### After (@sebastienrousseau/shell-config Enforced)

```text
# .shellcheckrc
enable=all
severity=style
# Hardens against SC2086 (unquoted vars), SC2154 (unreferenced), SC2164 (cd failures)
```
...
When maintaining legacy csh/tcsh shell scripts not supported by ShellCheck and shfmt.
