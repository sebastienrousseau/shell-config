...
describe("@sebastienrousseau/shell-config Unit Tests", () => {
  it("should load CommonJS module successfully", () => {
    const config = require("../index.cjs");
    assert(config !== null && typeof config === "object");
    assert.strictEqual(config.name, "@sebastienrousseau/shell-config");
  ...
  it("should load ESM module entrypoint successfully", async () => {
    ...
    assert.strictEqual(esm.default.name, "@sebastienrousseau/shell-config");
  });

  it("should have valid package.json metadata", () => {
    const pkg = JSON.parse(fs.readFileSync(path.join(__dirname, "../package.json"), "utf8"));
    assert.strictEqual(pkg.name, "@sebastienrousseau/shell-config");
