...
%global npm_bare      shell-config
%global sitelib       %{_prefix}/lib/node_modules

Name:           nodejs-sebastienrousseau-shell-config
Version:        0.0.7
Release:        1%{?dist}
Summary:        Shareable Shell / Bash configuration providing standardized ShellCheck and shfmt rules adhering to modern 2026 standards

License:        Apache-2.0 OR MIT
URL:            https://github.com/sebastienrousseau/shell-config
Source0:        %{name}-%{version}.tar.gz
BuildArch:      noarch
Requires:       nodejs >= 20

%description
@sebastienrousseau/shell-config is a shareable configuration preset with no runtime
...
install -pm 0644 .shellcheckrc %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
install -pm 0644 .shfmt %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
