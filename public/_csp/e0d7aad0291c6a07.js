...
lp=lm?'/shell-config/'+lm[1]:'/shell-config';
function load(){if(idx)return Promise.resolve();var sp=lm?'/shell-config/'+lm[1]+'/search-index.json':'/shell-config/search-index.json';return fetch(sp).then(function(r){return r.json()}).then(function(d){idx=d.entries||[]}).catch(function(){idx=[]})}
